...
const variants: Record<Variant, string> = {
  brand: "pill-gradient border-2 border-brand-dark bg-brand text-brand-foreground shadow-action hover:bg-brand-dark",
  primary: "border-2 border-charcoal bg-primary text-primary-foreground hover:bg-primary-dark",
  secondary: "pill-gradient border border-brand-muted bg-brand-soft text-brand shadow-soft hover:bg-brand/10",
  outline: "border-2 border-border bg-card text-foreground shadow-soft hover:bg-muted",
...
export function Button({
  ...
  const styles = cn(
    ...
    shape === "pill" ? "radius-control" : "rounded-md",
    variants[variant],
    sizes[size],
    className,
  );
