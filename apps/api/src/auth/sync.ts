...
function verifiedEmail(user: SupabaseAuthUser, identity: SupabaseIdentity): string | undefined {
  ...
  const providerVerified =
    data.email_verified === true || data.email_verified === "true" || data.email_verified === 1;
...
/**
 * Derive only routable identities proven by Supabase OAuth / email metadata.
 */
export function collectVerifiedIdentities(user: SupabaseAuthUser): VerifiedIdentity[] {
  ...
  const hasNonXIdentity = oauth.some(
    (identity) => identity.provider === "google" || identity.provider === "email",
  );
  ...
  for (const identity of oauth) {
    ...
    if (identity.provider === "email") {
      const email = verifiedEmail(user, identity) ??
        (user.email && user.email_confirmed_at ? normalizeIdentifier("email", user.email) : undefined);
      if (email) rows.push({ provider: "email", providerSubject: subject, normalizedIdentifier: email });
      continue;
    }
    ...
    if (handle) {
      rows.push({
        ...
        normalizedIdentifier: normalizeIdentifier("x", handle),
      });
    }
    ...
    if (email) {
      rows.push({
        provider: "email",
        providerSubject: `x-email:${subject}`,
        normalizedIdentifier: email,
      });
    }
  }

  // Email OTP users may lack an identities[] email row but have confirmed email.
  if (user.email && user.email_confirmed_at && !rows.some((row) => row.provider === "email" || row.provider === "google")) {
    rows.push({
      provider: "email",
      providerSubject: `email:${user.id}`,
      normalizedIdentifier: normalizeIdentifier("email", user.email),
    });
  }
  ...
  return rows.filter(
    (row, index) =>
      rows.findIndex(
        (candidate) =>
          candidate.provider === row.provider && candidate.providerSubject === row.providerSubject,
      ) === index,
  );
}
...
async function upsertVerifiedIdentity(
  ...
  if (bySubject && bySubject.profile_id !== profileId) throw new Error("identity_already_linked");
  ...
  if ((byIdentifier ?? []).some((row) => row.profile_id !== profileId)) {
  ...
  if (error) {
    if (error.code === "23505") throw new Error("identity_already_linked");
    throw error;
  }
}
...
export async function syncProfileIdentities(db: Db, token: string) {
  ...
  const displayName = stringValue(user.user_metadata?.full_name) ?? stringValue(user.user_metadata?.name) ?? null;
  const avatarUrl = stringValue(user.user_metadata?.avatar_url) ?? null;
